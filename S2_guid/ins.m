...
while(length(abs(err)>threthhold)>100 && ttcnt>0)
    ...
    thr=(max(max(expI))-min(min(expI)))/10;
...
path=strcat(imgDest,'scribble/',img(24:end),'p.bmp');
imwrite(scrib,path);
...
for i = 1:spnum
    ...
    if sum(sum(scrib.*flag))==0
        ...
        yfmax = floor(max(yf));
        ymean = floor(mean(yf));
        for fi=max(xfmin,xmean-wd):min(xmean+wd,xfmax)
              for fj=max(yfmin,ymean-wd):min(ymean+wd,yfmax)
                  scrib(fi,fj)=1;
              end
        end
    end
...
figure,imshow(scrib)
path=strcat(imgDest,'scribble/',img(24:end),'newp.bmp');
imwrite(scrib,path);
