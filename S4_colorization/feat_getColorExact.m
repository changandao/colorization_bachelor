function [nI,snI]=feat_getColorExact(colorIm,ntscIm,img)
        ...
        for ii=max(1,i-wd):min(i+wd,n)
           for jj=max(1,j-wd):min(j+wd,m)
              ...
              if (ii~=i)|(jj~=j)
                 ...
                 featdif = featdif1;
                 gvals(tlen) = featdif;
                 %gvals(tlen) = abs(cfeat.resize_cfeat3(ii,jj)-gfeat.resize_gfeat3(ii,jj)); 
                 
              end
           end
        end
        %t_val = cfeat.resize_cfeat3(i,j)-gfeat.resize_gfeat3(i,j);
        t_val=ntscIm(i,j,1);
        gvals(tlen+1)=t_val;
