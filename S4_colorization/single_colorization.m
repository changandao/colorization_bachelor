clear
addpath(genpath(pwd));
img = '../dataset/graysundatabase/canalandbank';
imgMid = '../dataset/sunscribble/';
imgDest = '../dataset/sunresult/';
g_name=strcat(img,'g.bmp');
c_name=strcat(img,'r.bmp');
cl_name=strcat(imgMid,'scribble/',img(28:end),'newp.bmp');
out_name=strcat(imgDest,img(28:end),'_oldout.bmp');
...
gfeatname=strcat(imgMid,'feature/',img(28:end),'_gfeatdata.mat');
cfeatname=strcat(imgMid,'feature/',img(28:end),'_cfeatdata.mat');
...
for j=1:m
   for i=1:n
      ...
      if (~colorIm(i,j))    
          ...
          gvals(1+tlen) = gcur;
          gc_var=mean((gvals(1:tlen)-gcur).^2);
          gcsig=gc_var*0.6;
